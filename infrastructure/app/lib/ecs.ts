...
export class EcsStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: Props & cdk.StackProps) {
    ...
    const lb = new elb.ApplicationLoadBalancer(this, "LaravelALB", {vpc, internetFacing: true})
    const listener = lb.addListener("LaravelListener", {port: 80});
    const targetGroup = listener.addTargets("LaravelEcsTG", {
      port: 80,
      targets: [ecsService]
    })
